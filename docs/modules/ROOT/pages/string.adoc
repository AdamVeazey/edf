...
:mutability_description: Many operations have a mutating and non-mutating version. The naming convention is to have the mutating version be named "presentTenseOperation" and the non-mutating version be named "getPastTenseOperations". For example, <<append>> and <<get_appended>>.
...
=== <<const_char_ref,String( const_char_ref )>>
.Example
[source,indent=0,c++]
----
include::{path_example_edf_string_main_cpp}[tag=init_str_literal_char]
...
=== <<const_const_uint8_t_ref,String( const const_uint8_t_ref )>>
.Example
[source,indent=0,c++]
----
include::{path_example_edf_string_main_cpp}[tag=init_str_literal_uint8_t]
