...
:size_t_literal: https://en.cppreference.com/w/cpp/language/user_literal[user-defined literal] to get a std::size_t. See <<important_size_t,IMPORTANT std::size_t>>. Defined in {ref_edf_math}.
...
[#important_size_t]
IMPORTANT: When using overloaded member functions that use either index/position, when using an index of `0` will attempt to use the position version of the member function. To work around this, use `0_uz` instead which is a {size_t_literal}
...
----
<1> Index usage, for 0, us a {size_t_literal}
<2> Iterator usage
...
----
<1> Index usage to insert 2 `0`'s at the beginning of the vector. For an index of 0, use a {size_t_literal}
...
include::{path_example_edf_vector_main_cpp}[tag=init]
    ...
    vector.insert( vector.begin(), { 0 } ); // <2>
----
<1> Index usage. For an index of 0, use a {size_t_literal}
<2> Iterator usage
...
----
<1> Constructs a `CustomType` at the first index of the vector. For index 0 use a {size_t_literal}
