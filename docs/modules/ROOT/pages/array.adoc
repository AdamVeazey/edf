...
----
<1> `index` is checked with {ref_module_ROOT}:assert.adoc#EDF_ASSERTD[EDF_ASSERTD] to be sure index is valid. In this example, it will cause `abort()` to be called.
