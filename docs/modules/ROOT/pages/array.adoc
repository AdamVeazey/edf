...
== Initialization
Initialization requires type `T` to be default constructable and copyable. 
...
----
include::{path_example_edf_array_main_cpp}[tag=init_POD]
include::{path_example_edf_array_main_cpp}[tag=element_access_at] <1>
----
<1> If uncommented, `index` is checked with {ref_module_ROOT}:assert.adoc#EDF_ASSERTD[] to be sure index is valid. In this example, it will cause `abort()` to be called.
...
=== operator==( lhs, rhs )
Compare two arrays to see if each element in both arrays are equal.
