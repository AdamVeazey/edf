...
inline constexpr std::size_t nElements( const T (&)[N] ) { return N; }
...
inline constexpr T voltageDividerCalcVout( const T& Vin ){
...
template<std::size_t R1, std::size_t R2, typename T>
inline constexpr T voltageDividerCalcVin( const T& Vout ){
...
template<typename T>
inline constexpr T temperatureCtoF( const T& temp_C ) {
...
template<typename T>
inline constexpr T temperatureFtoC( const T& temp_F ) {
